...
async def download_video(message: types.Message, bot) -> None:
    ...
    try:
        if len(audio_video_url) == 1:
            ...
            os.rename(f'youtube/{user_id}_{yt.title}.mp4', f'youtube/{user_id}_{yt.title}.mp3')
            with open(f'youtube/{user_id}_{yt.title}.mp3', 'rb') as audio:
                await bot.send_audio(user_id, audio, caption=f'*Готово. Ваше видео*: *{yt.title}*',
                                     parse_mode='Markdown')
                os.remove(f'youtube/{user_id}_{yt.title}.mp3')
        else:
        ...
        await bot.send_message(message.chat.id, '*Ограничение!!! Лимит на закачку ботом 50MB*.',
                               parse_mode='Markdown')
        if os.path.exists(f'youtube/{user_id}_{yt.title}.mp4'):
            os.remove(f'youtube/{user_id}_{yt.title}.mp4')
        elif os.path.exists(f'youtube/{user_id}_{yt.title}.mp3'):
            os.remove(f'youtube/{user_id}_{yt.title}.mp3')
        logger.error(error)
